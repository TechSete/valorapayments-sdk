...
/**
 * Configuração do cliente HTTP reativo para o SDK ValoraPayments.
 * <p>
 * Esta classe fornece a configuração necessária para a comunicação com a API da ValoraPayments,
 * criando e configurando uma instância de {@link WebClient} do Spring WebFlux.
 * </p>
 *
 * O WebClient é configurado com:
 * <ul>
 *   <li>URL base da API ValoraPayments</li>
 ...
 * com os endpoints da API ValoraPayments, fornecendo uma interface reativa para
 * requisições HTTP.
 * </p>
 *
 * @author Edson Isaac
 */
@Slf4j
@Configuration("valoraPaymentsWebClientConfiguration")
public class WebClientConfiguration {

    /**
     * Cria e configura um {@link WebClient} para comunicação com a API ValoraPayments.
     * <p>
     * Este bean cria uma instância de WebClient configurada especificamente para
     * interagir com a API da ValoraPayments. A configuração inclui:
     * </p>
     * <ul>
     *   <li>URL base da API: https://valora.apixpag.com/api/v1/rf</li>
     ...
     * requisições HTTP reativas à API da ValoraPayments.
     * </p>
     *
     * @return uma instância configurada de {@link WebClient}
     */
    @Bean(name = "valoraPaymentsWebClient")
    public WebClient ValoraPaymentsWebClient() {

        return WebClient.builder()
                .baseUrl("https://valora.apixpag.com/api/v1/rf")
    ...
    private Mono<ClientResponse> handleErrors(ClientResponse response) {
        if (response.statusCode().is4xxClientError() || response.statusCode().is5xxServerError()) {
            return response.bodyToMono(String.class)
                    .flatMap(errorBody -> {
                        log.error("Erro HTTP {} na chamada à API ValoraPayments: {}", response.statusCode().value(), errorBody);
                        return Mono.error(new RuntimeException("Erro ao chamar API ValoraPayments: " + errorBody));
                    });
        }
